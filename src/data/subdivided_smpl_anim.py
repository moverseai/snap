...
try:
    ...
    log.warning("pyarrow is not installed, please install it to use.")
...
class SubdividedSMPLAnim(torch.utils.data.Dataset):
    def __init__(
        self,
        anim_path: str,
        path: str,
        ...
        self.parquet_table = pq.read_table(anim_path).to_pandas()
        # betas , pose, transl, global_orient, should be replaced by the parquet table
        self.betas = np.stack(self.parquet_table["betas"].values)[
            0
...
if __name__ == "__main__":
    root = "//NAS5DD7C0/mov-founders/shared-founders/recordings/tpose-avatars/28f9a8d5-aebe-4773-b589-fb96e431a325"
    subdiv = SubdividedSMPLAnim(
        anim_path=f"{root}/bundle_rtm.anim",
        path=root,
        gender="male",
        models_path="D:/repos/gists/data/body_models/tools/clean_ch",
        # offsets_path="data/offsets.npz",
        # subset=[0],
        # level=1,
    )
